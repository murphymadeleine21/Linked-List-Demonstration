...
int main(){
    ...
    Sequence m = q;         //testing copy constructor
    
    assert(q.remove("hi") == 1);
  ...
  //for ItemType = unsigned long
    ...
    Sequence result;            //check interleave, with diff sizes, first bigger
    ...
    Sequence result2;
    interleave(a, b, result2);      //check interleave  with diff sizes, second bigger
    ItemType blah;
    ...
    Sequence result3;           //check interleave
    interleave(c, d, result3);          //with same size
    ItemType woo;
    ...
    Sequence result4;       //check interleave
    interleave(e, f, result4);          //with first empty
    ItemType ya;
    assert(result4.get(0, ya)  &&  ya == 1);
    assert(result4.get(1, ya)  &&  ya == 2);
    
    Sequence result5;
    interleave(f, e, result5);          //with second empty
    ItemType hi;
    assert(result5.get(0, hi)  &&  hi == 1);
    assert(result5.get(1, hi)  &&  hi == 2);
    
    Sequence h;
    Sequence resultempty;           //with two empty
    interleave(e, h, resultempty);
    ItemType empty;
    ...
    Sequence x2 = x;         //testing copy constructor
    ItemType ab;
    assert(x2.get(0, ab) && ab == 0);
    assert(x2.get(1, ab) && ab == 1);
    
  
    Sequence o;             //testing assignment operator
    o = x2;
    ItemType ba;
    ...
    ItemType z;
    assert(yz.remove(1) == 1 && yz.size() == 5);        //check remove
    assert(yz.get(1, z) && z == 2);
    assert(yz.remove(5) == 1 && yz.size() ==4);     //remove last element
    assert(yz.get(3, z) && z == 4);
    assert(yz.remove(0) == 1 && yz.size() == 3);    //remove first element
    ...
    op.swap(pi);            //check swap, same size
    ...
    op.swap(ii);            //check swap, diff sizes
